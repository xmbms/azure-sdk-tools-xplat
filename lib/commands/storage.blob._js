...
exports.init = function(cli) {
  ...
  */
  function uploadAzureBlob(file, containerName, blobName, options, _) {
    ...
    storageOptions.blobType = BlobConstants.BlobTypes[blobTypeName.toUpperCase()];
    ...
    var readStream = fs.createReadStream(specifiedFileName);
    ...
    try {
      if (blobTypeName.toLowerCase() == 'page') {
        ...
        operation = getStorageBlobOperation('createBlobPagesFromStream');
        performStorageOperation(operation, _, specifiedContainerName, specifiedBlobName, readStream, 0, fsStatus.size - 1, storageOptions);
      } else {
        //Upload block blob
        operation = getStorageBlobOperation('createBlockBlobFromStream');
        performStorageOperation(operation, _, specifiedContainerName, specifiedBlobName, readStream, fsStatus.size, storageOptions);
      }
    } finally {
      endProgress();
    }
